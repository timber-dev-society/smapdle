...
const Marker = ({ uid }) => {
  const { skin, position, isOver, token } = useSelector(state => state.markers.recorder[uid], isEqual)
  ...
  return createPortal(
    <Container className={`${isOver ? 'focus' : ''}`}>
      <Wrapper className={`zoom v-token`} onClick={() => handleOpenMenu()}>
        <Emoji symbol={getSkin(skin)} label="location" />
      </Wrapper>
...
export const recorderMarker = memo(Marker)
