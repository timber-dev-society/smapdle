...
const Marker = ({ uid }) => {
  const { skin, position, isOver, token } = useSelector(state => state.markers.actor[uid], isEqual)
  ...
  return createPortal(
    <Container className={`${isOver ? 'focus' : ''}`}>
      <Wrapper className={`zoom v-token`} onClick={() => handleOpenMenu()}>
        <Emoji symbol={getSkin(skin)} label="actors" />
      </Wrapper>
...
export const actorMarker = memo(Marker)
