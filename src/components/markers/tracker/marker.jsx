...
const Marker = ({ uid }) => {
  const { skin, position, isOver, token } = useSelector(state => state.markers.tracker[uid], isEqual)
  ...
  return createPortal(
    <Container className={`${isOver ? 'focus' : ''}`}>
      <Wrapper className={`zoom v-token`} onClick={() => handleOpenMenu()}>
        <Emoji symbol={getSkin(skin)} label="tracker" />
      </Wrapper>
...
export const trackerMarker = memo(Marker)
